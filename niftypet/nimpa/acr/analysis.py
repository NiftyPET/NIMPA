...
def standard_analysis(
    ...
    out = dict(h1max=h1, h2max=h2, h3max=h3, h4max=h4,
                bckg_avg=bckg_avg, bone_avg=bone_avg,
                h2o_avg=h2o_avg, air_avg=air_avg,
                bckg_min=bckg_min, bone_min=bone_min,
                h2o_min=h2o_min, air_min=air_min,
                h1_bckg=h1_bckg, h2_bckg=h2_bckg,
                h3_bckg=h3_bckg, h4_bckg=h4_bckg,
                h2_h1=h2_h1, h3_h1=h3_h1, h4_h1=h4_h1,
                air_bone=air_bone, h2o_bone=h2o_bone)
    ...
    out['test_bckg_avg'] ='PASS'*test1+'FAIL'*~test1 + ': '+str(round(bckg_avg,2)) +' (0.85-1.15)'
    out['test_25mm_insert'] = 'PASS'*test2+'FAIL'*~test2 + ': '+str(round(h1,2)) +' (1.8-2.8)'
    out['test_16/25_insert'] = 'PASS'*test3+'FAIL'*~test3 + ': '+str(round(h2_h1,2)) +' (>0.7)'
...
def estimate_fwhm(fim, vois, Cntd, insert='water'):
    ...
    res = dict( y=np.zeros(len(x), dtype=np.float64),
                dy=np.zeros(len(x), dtype=np.float64),
