...
def get_paths(Cntd, outpath=None):
    ...
    if 'fqntup' in Cntd and Path(Cntd['fqntup']).is_file():
        ...
        raise ValueError('NAC or QNT PET image has to be identified in the constants dictionary Cntd')
    ...
    facrad = os.path.join(t_acr_core_flddr, 'acr-activity-'+str(Cntd['vxsz']*Cntd['scld']).replace('.','-')+'mm.nii.gz')

    #> output file for the ACR mu-map without resolution rod insert
    facrmu = os.path.join(t_acr_core_flddr, 'acr-umap-'+str(Cntd['vxsz']).replace('.','-')+'mm.nii.gz')

    #> the same as above but with lower resolution (double voxel size)
    facrdmu = os.path.join(t_acr_core_flddr, 'acr-umap-'+str(Cntd['vxsz']*Cntd['scld']).replace('.','-')+'mm.nii.gz')
    ...
    fresomu = os.path.join(t_acr_reso_flddr, 'acr-reso-umap-'+str(Cntd['vxsz']).replace('.','-')+'mm.nii.gz')

    #> the same but double voxel size (as defined by scld)
    fresdmu = os.path.join(t_acr_reso_flddr, 'acr-reso-umap-'+str(Cntd['vxsz']*Cntd['scld']).replace('.','-')+'mm.nii.gz')

    #> the resolution rods mu-map with water background (used for the phantom)
    fresdWmu = os.path.join(t_acr_reso_flddr, 'acr-reso-water-umap-'+str(Cntd['vxsz']*Cntd['scld']).replace('.','-')+'mm.nii.gz')

    #> the resolution active QNT for alternative registration of the rods to PET
    fresdQmu = os.path.join(t_acr_reso_flddr, 'acr-reso-active-qnt-'+str(Cntd['vxsz']*Cntd['scld']).replace('.','-')+'mm.nii.gz')

    if 'fqntup' in Cntd and Path(Cntd['fqntup']).is_file():
        #> output file for the resolution rods part only
        fpet_res = os.path.join(rpth, os.path.basename(fimup).split('-scale-'+str(Cntd['sclt'])+'-')[0]+'_rods.nii.gz')
    ...
    fst_res = os.path.join(t_acr_smpl_flddr, 'acr-res-sampling-'+str(Cntd['vxsz']).replace('.','-')+'mm.nii.gz')
    #> sampling template for the inserts
    fst_insrt = os.path.join(t_acr_smpl_flddr, 'acr-all-sampling-'+str(Cntd['vxsz']).replace('.','-')+'mm.nii.gz')
    #> sampling template for the 3rd hot insert
    fst_insrt3 = os.path.join(t_acr_smpl_flddr, 'acr-insrt3-sampling-'+str(Cntd['vxsz']).replace('.','-')+'mm.nii.gz')
    #> sampling template for the insert background
    fst_ibckg = os.path.join(t_acr_smpl_flddr, 'acr-ibckg-sampling-'+str(Cntd['vxsz']).replace('.','-')+'mm.nii.gz')
...
def plot_hotins(
    ...
    if draw_bckg:
        for i in inserts:
            rect = patches.Rectangle((rinsrt[i-1],ylim[0]), 1.5, ylim[1], linewidth=0, edgecolor='None', facecolor='0.85')
            ax.add_patch(rect)
...
def plot_coldins(
    ...
    if draw_bckg:
        rect = patches.Rectangle((0,0),12.5, ylim[1],linewidth=0,edgecolor='None',facecolor='b', alpha=0.1)
        #> add the patch to the plot
        axs[0].add_patch(rect)
        rect = patches.Rectangle((12.5,0),1.5, ylim[1],linewidth=0,edgecolor='None',facecolor=str(0.8))
        #> add the patch to the plot
        axs[0].add_patch(rect)
        rect = patches.Rectangle((14,0),11, ylim[1],linewidth=0,edgecolor='None',facecolor='red', alpha=0.1)
        #> add the patch to the plot
        axs[0].add_patch(rect)
    ...
    if draw_bckg:
        rect = patches.Rectangle((0,0),12.5,ylim[1],linewidth=0,edgecolor='None',facecolor='None')
        #> add the patch to the plot
    ...
    if draw_bckg:
        rect = patches.Rectangle((0,0),12.5,ylim[1],linewidth=0,edgecolor='None',facecolor='0.85')
        axs[2].add_patch(rect)
        rect = patches.Rectangle((12.5,0),12.5,ylim[1],linewidth=0,edgecolor='None',facecolor='red', alpha=0.1)
        axs[2].add_patch(rect)
...
def plot_uniformity(
    ...
    if ring_drw_ymax is not None and ring_draw_ymin is not None:
        for k in range(len(Cntd['rbckgs'])):
            rect = patches.Rectangle((Cntd['rbckgs'][k]-6,ring_draw_ymin),6,ring_drw_ymax,linewidth=0,edgecolor='None',facecolor=str(0.8+(k%2)*0.1))
            #> add the patch to the plot
            ax.add_patch(rect)
...
def plot_rods(
    ...
    #> sampling
    for k, (nrng, off, rad) in enumerate(zip(
            Cntd['rods_nrngs'][pick_rods],
            Cntd['rods_off'][pick_rods],
