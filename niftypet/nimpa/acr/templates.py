...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
def create_mumap_core(
        Cntd,
        return_raw=False):


    if os.path.isfile(Cntd['out']['facrmu']) and os.path.isfile(Cntd['out']['facrdmu']):
        print('i> found already generated mu-map at:\n{}\nand\n{}'.format(Cntd['out']['facrmu'], Cntd['out']['facrdmu']))
        return None
    ...
    #-----------------------------------------------------------------------
    #> main cap, part 1 (B)
    mcap1 = np.float32((mcap1_png[...,0]==Cntd['png_prspx'])*Cntd['mu_prspx']) + \
            np.float32((mcap1_png[...,0]==Cntd['png_scrws'])*Cntd['mu_screw'])
    mcap1 = ndi.zoom(mcap1, Cntd['scl'], output=None, order=Cntd['intord'], mode='constant')
    #matshow(mcap1, cmap='bone', vmin=0.07, vmax=0.14)
    #-----------------------------------------------------------------------
    #-----------------------------------------------------------------------
    #> main cap, part 2 (C)
    mcap2 = np.float32((mcap2_png[...,0]==Cntd['png_prspx'])*Cntd['mu_prspx']) + \
            np.float32((mcap2_png[...,0]==Cntd['png_water'])*Cntd['mu_water'])
    # prnt = mcap2
    ...
    #-----------------------------------------------------------------------
    #> lid in two parts
    lid0 =  np.float32((lid0_png[...,0]==Cntd['png_scrws'])*Cntd['mu_screw']) +\
            np.float32((lid0_png[...,0]==Cntd['png_prspx'])*Cntd['mu_prspx']) +\
            np.float32((lid0_png[...,0]==Cntd['png_water'])*Cntd['mu_water'])
    lid0 = ndi.zoom(lid0, Cntd['scl'], output=None, order=Cntd['intord'], mode='constant')
    #matshow(lid0, cmap='bone', vmin=0.07, vmax=0.14)
    #-----------------------------------------------------------------------
    #-----------------------------------------------------------------------
    lid1 =  np.float32((lid1_png[...,0]==Cntd['png_scrws'])*Cntd['mu_screw']) +\
            np.float32((lid1_png[...,0]==Cntd['png_prspx'])*Cntd['mu_prspx']) +\
            np.float32((lid1_png[...,0]==Cntd['png_water'])*Cntd['mu_water'])
    ...
    #-----------------------------------------------------------------------
    #> cylindrical inserts
    insrt = np.float32((insrt_png[...,0]==Cntd['png_prspx'])*Cntd['mu_prspx']) + \
            np.float32((insrt_png[...,0]==Cntd['png_boney'])*Cntd['mu_bone'])  + \
            np.float32((insrt_png[...,0]==Cntd['png_water'])*Cntd['mu_water'])
    insrt = ndi.zoom(insrt, Cntd['scl'], output=None, order=Cntd['intord'], mode='constant')
    #matshow(insrt, cmap='bone', vmin=0.07, vmax=0.14)
    #-----------------------------------------------------------------------
    #-----------------------------------------------------------------------
    #> bone insert
    insrtb = np.float32((binsrt_png[...,0]==Cntd['png_prspx'])*Cntd['mu_prspx']) + \
             np.float32((binsrt_png[...,0]==Cntd['png_water'])*Cntd['mu_water'])
    ...
    #-----------------------------------------------------------------------
    #> main compartment 
    main = np.float32((main_png[...,0]==Cntd['png_prspx'])*Cntd['mu_prspx']) + \
           np.float32((main_png[...,0]==Cntd['png_water'])*Cntd['mu_water'])
    ...
    acr = np.concatenate(
            (mcapA, mcapB, mcapC, mcapD, lidA, lidB, insrtA, insrtB, contA, contB),
            axis=0)

    #> pad the image to make the dims even
    acr = np.pad( acr, ((0,0), (Cntd['dpad'],Cntd['dpad']+1),(Cntd['dpad'],Cntd['dpad']+1)), constant_values=((0,0), (0,0), (0, 0)) )
    ...
    affine = np.array([ [-Cntd['vxsz'], 0., 0., .5*imxys*Cntd['vxsz']],
                        ...
                        [0., 0., Cntd['vxsz'], -.5*imzys*Cntd['vxsz']],
                        [0., 0., 0., 1.]])
    ...
    nimpa.array2nii(
        ...
        affine,
        Cntd['out']['facrmu'],
        trnsp = (imupd['transpose'].index(0), imupd['transpose'].index(1), imupd['transpose'].index(2)),
        flip = imupd['flip'])
    ...
    imxys = acrd.shape[2]
    imzys = acrd.shape[0]
    affined = np.array([[-Cntd['vxsz']*Cntd['scld'], 0., 0., .5*imxys*Cntd['vxsz']*Cntd['scld']],
    ...
    #del insrt, insrtb, insrtA, insrtB, contA, contB, main, lidA, lidB, lid0, lid1, mcapA, mcapB, mcapC, mcapD
...
def create_nac_core(
    ...
    ins =   np.float32((ins_png[...,0]==Cntd['png_abck'])*Cntd['abck']) +\
            np.float32((ins_png[...,0]==Cntd['png_ains'])*Cntd['ains'])
    ...
    #-----------------------------------------------------------------------
    k = int(np.round(Cntd['k_mcapA']/Cntd['vxsz'])) + int(np.round(Cntd['k_mcapB']/Cntd['vxsz'])) + int(np.round(Cntd['k_mcapC']/Cntd['vxsz']))
    ...
    k = int(np.round(Cntd['k_unfrm']/Cntd['vxsz'])) + int(np.round(Cntd['k_insrtB']/Cntd['vxsz']))
    ...
    #> 3D assembly
    acra = np.concatenate(
            (cap_, capa, lid_, insa, biga, gapa, resa, gapa, btm_),
            axis=0)

    #> pad the image to make the dims even
    acra = np.pad( acra, ((0,0), (Cntd['dpad'],Cntd['dpad']+1),(Cntd['dpad'],Cntd['dpad']+1)), constant_values=((0,0), (0,0), (0, 0)) )
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#> RESOLUTION RODS MU-MAPs and REGISTRATION TEMPLATE 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
def create_reso(Cntd, return_raw=False):
    ...
    if os.path.isfile(Cntd['out']['fresomu']) and os.path.isfile(Cntd['out']['fresdmu']) \
        and os.path.isfile(Cntd['out']['fresdWmu']) and os.path.isfile(Cntd['out']['fresdQmu']):
        return None
    ...
    renW = (renWpng[...,0]==Cntd['png_water'])*Cntd['mu_water'] + (renWpng[...,0]==Cntd['png_prspx'])*Cntd['mu_prspx']
    resW = (resWpng[...,0]==Cntd['png_water'])*Cntd['mu_water'] + (resWpng[...,0]==Cntd['png_prspx'])*Cntd['mu_prspx']
    ...
    if Cntd['rods_rotate']!=0:
        ren = ndi.rotate(ren, Cntd['rods_rotate'], reshape=False, order=Cntd['intord'], mode='constant')
        res = ndi.rotate(res, Cntd['rods_rotate'], reshape=False, order=Cntd['intord'], mode='constant')

        renW = ndi.rotate(renW, Cntd['rods_rotate'], reshape=False, order=Cntd['intord'], mode='constant')
        resW = ndi.rotate(resW, Cntd['rods_rotate'], reshape=False, order=Cntd['intord'], mode='constant')
    ...
    #> create a buffer for registration purposes as a margin between the rods end and the uniform part
    ...
    reso = np.pad( reso, ((1,0), (Cntd['dpad'],Cntd['dpad']+1),(Cntd['dpad'],Cntd['dpad']+1)), constant_values=((0,0), (0,0), (0, 0)) )
    ...
    resoW = np.pad( resoW, ((1,0), (Cntd['dpad'],Cntd['dpad']+1),(Cntd['dpad'],Cntd['dpad']+1)), constant_values=((0,0), (0,0), (0, 0)) )
    ...
    if return_raw:
        return dict(resoW=resoW, resdW=resdW, resd=resd, reso=reso)
...
def create_sampl_reso(
    ...
    gamma= 1.
    y = np.int16(0.2126*rods_png[...,0]**gamma + 0.7152*rods_png[...,1]**gamma + 0.0722*rods_png[...,2]**gamma)
    ...
    lvls = []
    lvls.append(uy[:k])
    lvls.append(uy[k:k+o]); k+=o
    lvls.append(uy[k:k+o]); k+=o
    lvls.append(uy[k:k+o]); k+=o; o=5
    lvls.append(uy[k:k+o]); k+=o
    lvls.append(uy[k:k+o]); k+=o; o=4
    lvls.append(uy[k:k+o]); k+=o; o=3
    lvls.append(uy[k:k+o]); k+=o
    lvls.append(uy[k:k+o]); k+=o; o=2
    lvls.append(uy[k:k+o]); k+=o; o=1
    lvls.append(uy[k:k+o]); k+=o
    lvls.append(uy[k:k+o]); k+=o
    ...
    if Cntd['rods_rotate']!=0:
        srods = ndi.rotate(srods, Cntd['rods_rotate'], reshape=False, order=Cntd['intord'], mode='constant')
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#> SAMPLING PATTERNS FOR INSERTS and BACKGROUND
...
def create_sampl(
    ...
    if os.path.isfile(Cntd['out']['fst_insrt']) and os.path.isfile(Cntd['out']['fst_insrt3']) \
        and os.path.isfile(Cntd['out']['fst_ibckg']):
        return None
    ...
    #> AXIAL SPACING
    #> upper padding
    ku = int(np.round(Cntd['k_mcapA']/Cntd['vxsz'])) + int(np.round(Cntd['k_mcapB']/Cntd['vxsz'])) \
    + int(np.round(Cntd['k_mcapC']/Cntd['vxsz'])) + int(np.round(Cntd['k_mcapD']/Cntd['vxsz'])) \
    + int(np.round(Cntd['k_lidA']/Cntd['vxsz'])) + int(np.round(Cntd['k_lidB']/Cntd['vxsz']))
    ...
    allsmplng = np.concatenate(
            (up_, insrts, ibtm_, bckg, btm_),
            axis=0)

    insrt3smplng = np.concatenate(
            (up_, insrt3, ibtm_, bckg, btm_),
            axis=0)

    ibckgsmplng = np.concatenate(
            (up_, ibckg, ibtm_, bckg, btm_),
            axis=0)

    #> pad the image to make the dims even
    allsmplng = np.pad(allsmplng, ((0,0), (0,1),(0,1)), constant_values=((0,0), (0,0), (0, 0)) )
    insrt3smplng = np.pad(insrt3smplng, ((0,0), (0,1),(0,1)), constant_values=((0,0), (0,0), (0, 0)) )
    ibckgsmplng = np.pad(ibckgsmplng, ((0,0), (0,1),(0,1)), constant_values=((0,0), (0,0), (0, 0)) )
    ...
    if return_raw:
        out = dict(
            allsmplng=allsmplng,
            insrt3smplng=insrt3smplng,
            ibckgsmplng=ibckgsmplng)
        return out
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
