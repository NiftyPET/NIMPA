...
srngs = np.array([
    2.52, 4.53, 6.54, 8.56, 10.65, 12.68, 14.71, 16.74, 18.8, 20.8, 24
    ])/2
...
def get_params(cpath=None):
    ''' Get all the parameters and file names for ACR template
        generation.
        Arguments:
    ...
    elif os.path.isdir(cpath) and os.path.isfile(os.path.join(cpath, 'core_mumap', 'acr-main-compartment.png')):
        mfldr = Path(cpath)
    else:
